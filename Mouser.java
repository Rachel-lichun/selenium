...
public class Mouser {
	...
	public static void main(String[] args) throws InterruptedException {
      ...
      //必须有等待时间，如果登录代码和定位输入框之间没有间隔时间，无法定位元素。
		try {
			Thread.sleep(5000);
		 ...
		 * ***.click()。（鼠标已在输入框处）模拟鼠标左键单击
		 * ***.keyDown(element, Keys.SHIFT) 模拟键盘按住shitf键
		 * ***.sendKeys(element, "ccaa")    模拟键盘，向element输入框 send 值
		 * ***.keyUp(element, Keys.SHIFT)   模拟键盘，释放shitf键
		 * ***.build().perform()    执行上述所有模拟操作
		 */
		action.moveToElement(element).click().keyDown(element, Keys.SHIFT)
				.sendKeys(element, "ccaa").keyUp(element, Keys.SHIFT)
				.perform();
	}
